...
export class KeyConverter {
	public static async getX509RSA(kp: any) {
		...
		return {
			jwk,
			der: undefined,
			pemAsPrivate: await (Rasha as any).export({ jwk: kp.toJSON(true) }),
			pemAsPublic: await (Rasha as any).export({ jwk: kp.toJSON() }),
			ldSuite
		};
	}
	...
	public static async getP256(kp: ec.KeyPair, passphrase?: string) {
		...
		const ldSuite = {
			...
			pubBytes: () =>
				ethers.utils.arrayify('0x' + kp.getPublic().encodeCompressed('hex')),
	...
	public static createLinkedDataJsonFormat(
		algorithm: LDCryptoTypes,
		key: KeyLike,
		hasPrivate = false
