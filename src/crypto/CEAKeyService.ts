...
export class CEAKeyService implements KeyService {
	public getPrivateKey(algorithm: AlgorithmType, keyStore: KeyModel): string {
		switch (algorithm) {
	...
	private getEd25519(mnemonic: string): eddsa.KeyPair {
	...
	private getRSA256Standalone(len: number = 2048): Promise<JWK.Key> {
	...
	public async generateKeys(
		...
		const keyExportED25519 = await KeyConverter.getEd25519(edKeypair);
		keyExportED25519.ldJsonPublic = KeyConverter.createLinkedDataJsonFormat(
		...
		const keyExportES256K = await KeyConverter.getES256K(es256kKeypair);
		keyExportES256K.ldJsonPublic = KeyConverter.createLinkedDataJsonFormat(
			LDCryptoTypes.JWK,
			// @ts-ignore
		...
		const keyExportRSA = KeyConverter.getRSA(rsaKeypair);
